...
def import_tea():
        ...
        with open('teadata.csv', newline='') as csvfile:
            ...
            for Name, Category, Feeling, Diseases, Regions, Rare, Caffenie, Description, Link, Pubmed in teareader:
                # tea_list.append(str.split(row))
                # tea_list.append([Name, Category, Feeling, Diseases.split(','), Regions.split(','), Rare, Caffenie])
                next_tea = tea.tea(Name, Category, Feeling, Diseases.split(','), Regions.split(','), Rare, (Caffenie == 'checked'), Description, Link, Pubmed)
